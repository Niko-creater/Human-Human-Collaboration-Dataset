...
_via_import_export.prototype.export_to_temporal_segments_csv = function() {
  return new Promise( function(ok_callback, err_callback) {
    ...
    for ( var mid in this.d.store.metadata ) {
      ...
      // ----- begin change -----
      var descriptive_av = {};
      var av = this.d.store.metadata[mid].av;
      for ( var aid in av ) {
        if ( this.d.store.attribute.hasOwnProperty(aid) ) {
          // use the human‐readable name
          var aname = this.d.store.attribute[aid].aname;
          descriptive_av[ aname ] = av[aid];
        }
        else {
          // fallback to raw aid if attribute is missing
          descriptive_av[ aid ] = av[aid];
        }
      }
      // ----- end change -----
