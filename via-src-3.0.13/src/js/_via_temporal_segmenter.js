...
// new
_via_temporal_segmenter.prototype._tmetadata_mid_add_at_timepoint = function(t) {
  var self = this;

  // 1. Pause playback so user can annotate this exact frame
  this.m.pause();
  _via_util_msg_show('Paused for text annotation', true);

  // 2. Build the time array [t, t]
  var z = [ t, t ];
  if (z[0] < 0) {
    z[0] = 0.0;
    z[1] = 0.0;
  ...
  // 3. Avoid duplicate at same time
  if (this.d.store.metadata.hasOwnProperty(this.metadata_last_added_mid)) {
    var prev = this.d.store.metadata[this.metadata_last_added_mid].z;
    if (prev[0] === z[0] && prev[1] === z[1]) {
      _via_util_msg_show('A temporal segment of same size already exists.');
      return;
    }
  }

  // 4. Prepare metadata with group-by attribute
  var xy = [];
  var metadata = {};
  metadata[this.groupby_aid] = this.selected_gid;

  // 5. Create the segment
  this.d.metadata_add(this.vid, z, xy, metadata)
    .then(function(ok) {
      var new_mid = ok.mid;
      ...
      // 7. Save the text into your TEXT attribute
      //    Replace 'attr5' with your actual TEXT attribute ID
      var TEXT_ANNOTATION_AID = 'attr5';
      return self.d.metadata_update_av(
        self.vid,
        new_mid,
        TEXT_ANNOTATION_AID,
        userText
    ...
    .then(function() {
      ...
      _via_util_msg_show('Text annotation saved', true);
    })
    .catch(function(err) {
...
_via_temporal_segmenter.prototype._on_event_keydown = function(e) {
  ...
  if ( (e.key === 'o') && ! e.ctrlKey ) {
    e.preventDefault();
    var t = this.m.currentTime;
    this._tmetadata_mid_add_at_timepoint(t);
    this._tmetadata_group_gid_draw(this.selected_gid); 
    return;
  }
